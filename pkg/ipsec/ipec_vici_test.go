...
func TestIpsecVersion(t *testing.T) {
	...
	assert.Equal(t, "5.9.13", resp.Version)
	assert.Equal(t, "Linux", resp.Sysname)
	assert.Equal(t, "6.1.112+", resp.Release)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want != nil {
				...
				assert.Equal(t, tt.want.Connection[0].Children[0].Name, resp.Connection[0].Children[0].Name)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want != nil {
				...
				assert.Equal(t, resp.Ikesas[0].InitiatorSpi, tt.want.Ikesas[0].InitiatorSpi)
				assert.Equal(t, resp.Ikesas[0].ResponderSpi, tt.want.Ikesas[0].ResponderSpi)
